...
from forms import GetMI, GetCI, GetMIByCI, AddMachine, CallLog, AddService
import psycopg2
	...
	if(form1.validate_on_submit() and ('submit' in request.form)):
		if(request.form['submit'] == 'login'):
			...
			if(len(ans) > 0):
				...
				st = sel.where(
					machines.c.machine_id == form1.machine_id.data
				)
				result = conn.execute(st)
				lis = []
				for row in result:
				...
				session['customer_id'] = lis[0][1]
				return redirect(url_for('calllog'))
			else:
				flash('Invalid Machine Serial Number!')
